...
When purchase tax is enabled, tax is posted as a separate entry to a mapped tax account.
*/
{% set using_purchase_tax_line = var('using_purchase_tax_line', False) %}
{% set using_tax_rate = var('using_tax_rate', False) if using_purchase_tax_line else False %}
...
purchase_tax_lines as (
    ...
    select
        purchase_id,
        source_relation,
        cast(index + 10000 as {{ dbt.type_string() }}) as index,
        tax_rate_id,
        amount
